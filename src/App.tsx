...
import { 
  PackageCheck, LogOut, LayoutDashboard, PlusCircle, Truck, Warehouse, 
  Bell, X, Server, PieChart, ShieldAlert, Users, Menu 
} from 'lucide-react';
  ...
  return (
    <div className="hidden md:block bg-white border-b border-slate-200 shadow-sm sticky top-[57px] z-40">
      <div className="max-w-7xl mx-auto px-4 flex space-x-2 py-2 overflow-x-auto">
        ...
        {userRole === 'ADMIN' && (
          <>
            <Link to="/siyanat-operations" className={getTabClass('/siyanat-operations')}><Truck className="w-4 h-4" /><span>Queue</span></Link>
...
const MobileDrawerNavigation = ({ userRole, isOpen, setIsOpen, handleLogout }: { userRole: string | null, isOpen: boolean, setIsOpen: (val: boolean) => void, handleLogout: () => void }) => {
  const location = useLocation();
    ...
    <div className="md:hidden fixed inset-0 z-50 bg-black/50 flex justify-end">
      <div className="w-72 bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right-8 duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          {userRole === 'ADMIN' && (
            <>
              <div className="pt-4 pb-2 text-[10px] font-black uppercase text-slate-400 tracking-wider">Admin Controls</div>
    ...
    <div className="min-h-screen flex flex-col bg-[#f8f6f0] font-sans">
      <header className="bg-brand-maroon text-white shadow-xl sticky top-0 z-50 border-b-2 border-brand-gold/40">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <button onClick={() => setIsMobileMenuOpen(true)} className="md:hidden p-2 bg-brand-dark rounded-lg">
  ...
  return (
    <Router>
      <Routes>
        ...
        {/* Admin Routes */}
        <Route path="/siyanat-operations" element={session && userRole === 'ADMIN' ? <PortalLayout userRole={userRole} userId={session.user.id}><SiyanatOperations /></PortalLayout> : <Navigate to="/" />} />
        <Route path="/restock" element={session && userRole === 'ADMIN' ? <PortalLayout userRole={userRole} userId={session.user.id}><RestockInventory /></PortalLayout> : <Navigate to="/" />} />
        <Route path="/reports" element={session && userRole === 'ADMIN' ? <PortalLayout userRole={userRole} userId={session.user.id}><Reports /></PortalLayout> : <Navigate to="/" />} />
        <Route path="/assets" element={session && userRole === 'ADMIN' ? <PortalLayout userRole={userRole} userId={session.user.id}><AssetRegister /></PortalLayout> : <Navigate to="/" />} />
        <Route path="/audit" element={session && userRole === 'ADMIN' ? <PortalLayout userRole={userRole} userId={session.user.id}><AuditLogs /></PortalLayout> : <Navigate to="/" />} />
        <Route path="/team" element={session && userRole === 'ADMIN' ? <PortalLayout userRole={userRole} userId={session.user.id}><TeamManagement /></PortalLayout> : <Navigate to="/" />} />
      </Routes>
    </Router>
  );
}