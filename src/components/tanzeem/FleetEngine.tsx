...
import { Car, MapPin, Calculator, XCircle, Send, X, Plus } from 'lucide-react';
...
export default function FleetEngine() {
  ...
  const fetchFleet = async () => {
    ...
    const [reqRes, invRes] = await Promise.all([
      supabase.from('vehicle_requests').select(`*, requester:profiles(full_name, department)`).eq('pipeline_state', 'AUTHORIZED').order('request_date', { ascending: true }),
  ...
  const addInternalVehicle = (v: FleetInventory) => {
    ...
    setAssignments([...assignments, { 
      ...
      type: 'INTERNAL', 
      vehicleId: v.id, 
      name: `${v.vehicle_type} (${v.license_plate})`, 
  ...
  const addRentalVehicle = () => {
    ...
    setAssignments([...assignments, { 
      id: crypto.randomUUID(), 
      type: 'RENTAL', 
      name: `Rental: ${customRentalName}`, 
      capacity: customRentalCapacity, 
      rounds: 1 
    }]);
    setCustomRentalName(''); 
    setCustomRentalCapacity(4);
  };
  ...
  if (loading) return <div className="p-8 text-center animate-pulse text-slate-500">Loading fleet requests...</div>;
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      ...
      {/* Fleet Rejection Modal (Replaces native prompt) */}
        ...
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleRejectSubmit} className="p-5 space-y-4">
              ...
              />
              <button type="submit" disabled={processingId === targetReq.id} className="w-full py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl uppercase text-xs tracking-wider transition disabled:opacity-50">
                Confirm Rejection
              </button>
            </form>
          </div>
