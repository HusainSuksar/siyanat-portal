...
import { Activity, Wrench, Package, Calendar, Clock, ArrowRight, Settings, LayoutDashboard } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const [adminView, setAdminView] = useState<'overview' | 'settings'>('overview');
  ...
  return (
    <div className="space-y-6 pb-24 max-w-6xl mx-auto animate-in fade-in duration-500">
      ...
      {/* Unified Maroon Welcome Banner with Settings Switcher */}
      <div className="bg-gradient-to-r from-brand-maroon to-brand-dark rounded-3xl p-6 md:p-8 text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-5 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3">
            <h2 className="text-2xl md:text-3xl font-black text-brand-gold tracking-tight">
              {adminView === 'overview' ? 'My Dashboard' : 'System Administration'}
            </h2>
          ...
          <p className="text-sm text-slate-200 mt-2 font-medium">
            {adminView === 'overview'
              ? 'Master System Overview & Health Metrics.'
              : 'Configure classes, headcount limits, trades, and categories.'}
          </p>
        </div>
        ...
        <div className="relative z-10">
          <button
            onClick={() => setAdminView(adminView === 'overview' ? 'settings' : 'overview')}
            className="px-4 py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-xl text-xs font-black uppercase tracking-wider transition border border-white/20 flex items-center gap-2 shadow-sm"
          >
            {adminView === 'overview' ? (
              <>
      ...
      {/* Conditionally Render Settings or Overview */}
      {adminView === 'settings' ? (
        <SystemConfigManager />
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div 
              className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between hover:border-amber-300 transition cursor-pointer" 
            ...
            <div 
              className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between hover:border-indigo-300 transition cursor-pointer" 
            ...
            <div 
              className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between hover:border-emerald-300 transition cursor-pointer" 
              onClick={() => navigate('/tanzeem')}
            >
              <div>
